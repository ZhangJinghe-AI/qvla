...
from qvla.core.quantize import channel_percentile_amax
...
class LayerStats:
    ...
    per_step_amax: torch.Tensor | None = None
    # Static (cross-step) amax — always populated, used for rotation / perm fit.
    static_amax: torch.Tensor = field(default_factory=lambda: torch.zeros(0))
    # Per-token |activation| samples (CPU) for per-channel percentile act scales.
    ...
    def init_phase1(self, in_features: int, num_steps: int, device, dtype) -> None:
        ...
        self.static_amax = torch.zeros(in_features, device=device, dtype=torch.float32)
        self.static_abs_samples = []
        self.per_step_abs_samples = None
        self.per_step_amax = None
        self.n_tokens = 0

    def update_phase1(self, x: torch.Tensor, *, step: int | None) -> None:
        """Accumulate ``xtx`` and ``static_amax`` from one mini-batch of activations."""
        flat = (
        ...
        torch.maximum(self.static_amax, amax_now, out=self.static_amax)
    ...
    def init_phase2(self, in_features: int, num_steps: int, device) -> None:
        ...
        self.static_abs_samples = []
        if num_steps > 1:
            self.per_step_amax = torch.zeros(
    ...
    def update_phase2(self, x: torch.Tensor, *, step: int | None) -> None:
        """Accumulate ``xtx``, ``static_amax``, and ``per_step_amax``."""
        flat = (
            x.reshape(-1, self.in_features)
            .detach()
            .to(device=self.xtx.device, dtype=torch.float32)
        ...
        if self.per_step_amax is not None and step is not None:
            row = self.per_step_amax[step]
    ...
    def empty(in_features: int) -> "LayerStats":
        ...
        ls.static_amax = torch.zeros(in_features, dtype=torch.float32)
        ls.static_abs_samples = []
        ls.per_step_abs_samples = None
        ls.per_step_amax = None
        ls.n_tokens = 0
        return ls

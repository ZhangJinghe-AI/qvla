#!/usr/bin/env python
r"""All-token median+3MAD band (normal-outlier) vs massive, by-step LP/HP.
...
* |x| > μ+3σ: massive
* MAD floor < |x| ≤ μ+3σ: normal-outlier
...
Example::
    ...
    CUDA_VISIBLE_DEVICES=5 HF_ENDPOINT=https://hf-mirror.com uv run python \
      tools/analyze_dit_outlier_lp_hp_channel_mad_by_step.py \
      ...
      --output-dir tools/img/dit_outlier_lp_hp_channel_mad_by_step_s16n4
"""
...
def _plot_metrics(rows: list[StepRow], cutoff: int, output: Path) -> None:
    ...
    fig.suptitle(
        "Normal = MAD-to-μ+3σ on selected channels  |  clip one denoise step",
        fontsize=12,
    )
...
def _plot_channel_counts(rows: list[StepRow], output: Path) -> None:
    ...
    ax.set_xticks(list(steps))
    ax.set_title("Channel all-token MAD band L1 match counts")
    ax.legend(fontsize=8)
    ax.grid(alpha=0.25)
    output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output, dpi=150)
...
def _experiment(
    ...
    *,
    ...
    clip_kw = dict(**run_kw, normal_ref_l1=True, normal_ref_mode="channel_mad")
    rows: list[StepRow] = []
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tools/img/dit_outlier_lp_hp_channel_mad_by_step"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"cutoffs={cutoffs} conditions={len(conditions)} clip=by_step channel_mad"
    )
    rows: list[StepRow] = []
    ...
    for line in lines:
        print(line)
    csv_path = args.output_dir / "outlier_lp_hp_channel_mad_by_step.csv"
    verdict_path = args.output_dir / "hypothesis.txt"
    _write_csv(rows, csv_path)
    verdict_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    for cutoff in cutoffs:
        _plot_metrics(rows, cutoff, args.output_dir / f"outlier_lp_hp_channel_mad_by_step_k{cutoff}.png")
    _plot_channel_counts(rows, args.output_dir / "outlier_lp_hp_channel_mad_by_step_counts.png")

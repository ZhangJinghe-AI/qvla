#!/usr/bin/env python
r"""Complementary-channel 3σ shoulder, clip one denoise step, score LP/HP.

Massive: MAD-selected channels, clip ``μ+3σ`` tails.
Shoulder: remaining channels, clip those columns' ``μ+3σ`` tails first, then
expand below 3σ largest-first until the removed L1 matches the massive total.
If remaining leftover cannot match, both sides skip that linear site.

Example::

    CUDA_VISIBLE_DEVICES=5 HF_ENDPOINT=https://hf-mirror.com uv run python \
      tools/analyze_dit_outlier_lp_hp_complement_l1_by_step.py \
      ...
      --output-dir tools/img/dit_outlier_lp_hp_complement_l1_by_step
"""
...
def _plot_metrics(rows: list[StepRow], cutoff: int, output: Path) -> None:
    ...
    fig.suptitle("Complement 3σ L1  |  clip one denoise step", fontsize=12)
...
def _plot_channel_counts(rows: list[StepRow], output: Path) -> None:
    ...
    ax.set_xticks(list(steps))
    ax.set_title("Complement L1 match counts")
    ax.legend(fontsize=8)
    ax.grid(alpha=0.25)
    output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output, dpi=150)
...
def _experiment(
    ...
    *,
    ...
    clip_kw = dict(**run_kw, complement_l1=True)
    rows: list[StepRow] = []
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tools/img/dit_outlier_lp_hp_complement_l1_by_step"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"cutoffs={cutoffs} conditions={len(conditions)} clip=by_step complement_l1"
    )
    rows: list[StepRow] = []
    ...
    for line in lines:
        print(line)
    csv_path = args.output_dir / "outlier_lp_hp_complement_l1_by_step.csv"
    verdict_path = args.output_dir / "hypothesis.txt"
    _write_csv(rows, csv_path)
    verdict_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    for cutoff in cutoffs:
        _plot_metrics(rows, cutoff, args.output_dir / f"outlier_lp_hp_complement_l1_by_step_k{cutoff}.png")
    _plot_channel_counts(rows, args.output_dir / "outlier_lp_hp_complement_l1_by_step_counts.png")

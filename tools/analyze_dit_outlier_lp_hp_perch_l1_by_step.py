#!/usr/bin/env python
r"""Per-channel matched L1, clip **one denoise step at a time**, score LP/HP.

Intervention matches ``analyze_dit_outlier_lp_hp_perch_l1.py`` (MAD channels,
per-channel L1, expand below MAD floor, skip starved channels on both sides).
Unlike that all-step run, each forward clips a single denoise step.

Scoring and plots follow ``analyze_dit_outlier_lp_hp_impact.py``.

Example::

    CUDA_VISIBLE_DEVICES=5 HF_ENDPOINT=https://hf-mirror.com uv run python \
      tools/analyze_dit_outlier_lp_hp_perch_l1_by_step.py \
      ...
      --output-dir tools/img/dit_outlier_lp_hp_perch_l1_by_step
"""
...
def _plot_metrics(rows: list[StepRow], cutoff: int, output: Path) -> None:
    ...
    fig.suptitle("Per-channel L1  |  clip one denoise step", fontsize=12)
...
def _plot_channel_counts(rows: list[StepRow], output: Path) -> None:
    ...
    ax.plot(xs, target, "k-o", label="has massive L1")
    ax.plot(xs, expand, "C1-s", label="expand enough")
    ax.plot(xs, short, "C3-^", label="insufficient skip")
    ax.set_xlabel("denoise step")
    ax.set_ylabel("channels / step (mean over conditions)")
    ax.set_xticks(list(steps))
    ax.set_title("Per-channel L1 match counts")
    ax.legend(fontsize=8)
    ax.grid(alpha=0.25)
    output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output, dpi=150)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tools/img/dit_outlier_lp_hp_perch_l1_by_step"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"cutoffs={cutoffs} conditions={len(conditions)} clip=by_step per_channel_l1"
    )
    rows: list[StepRow] = []
    ...
    for line in lines:
        print(line)
    csv_path = args.output_dir / "outlier_lp_hp_perch_l1_by_step.csv"
    verdict_path = args.output_dir / "hypothesis.txt"
    _write_csv(rows, csv_path)
    verdict_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    for cutoff in cutoffs:
        _plot_metrics(rows, cutoff, args.output_dir / f"outlier_lp_hp_perch_l1_by_step_k{cutoff}.png")
    _plot_channel_counts(rows, args.output_dir / "outlier_lp_hp_perch_l1_by_step_counts.png")

#!/usr/bin/env python
r"""Amax-ordered complementary 3σ tails, clip one denoise step, score LP/HP.

Massive: MAD-selected channels, clip ``μ+3σ`` tails.
Shoulder: remaining channels sorted by amax (high first); fully clamp each
column's ``μ+3σ`` tails until the massive L1 is reached; the last column is
scaled to match exactly. If remaining tails cannot reach the target, skip both.

Example::

    CUDA_VISIBLE_DEVICES=5 HF_ENDPOINT=https://hf-mirror.com uv run python \
      tools/analyze_dit_outlier_lp_hp_complement_amax_by_step.py \
      --checkpoint /data/share/pi05_libero_finetuned_v044 \
      --calibration-data ../calibration_data/libero_goal_30_7_demo.npz \
      --samples 0,1,3 --noise-seeds 0,1 \
      --output-dir tools/img/dit_outlier_lp_hp_complement_amax_by_step
"""
...
def _plot_metrics(rows: list[StepRow], cutoff: int, output: Path) -> None:
    ...
    fig.suptitle("Amax-greedy remaining 3σ  |  clip one denoise step", fontsize=12)
...
def _plot_channel_counts(rows: list[StepRow], output: Path) -> None:
    ...
    ax.plot(xs, expand, "C1-s", label="remaining ch fully clipped")
    ax.plot(xs, short, "C3-^", label="insufficient skip")
    ax.set_xlabel("denoise step")
    ax.set_ylabel("count / step (mean over conditions)")
    ax.set_xticks(list(steps))
    ax.set_title("Amax-greedy remaining 3σ counts")
    ax.legend(fontsize=8)
    ax.grid(alpha=0.25)
    output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output, dpi=150)
...
def _experiment(
    ...
    *,
    ...
    clip_kw = dict(**run_kw, complement_amax_l1=True)
    rows: list[StepRow] = []
        ...
        for cutoff in cutoffs:
            ...
            if cutoff == cutoffs[0]:
                print(
                    ...
                    f"ch full={massive_stats.n_ch_expand_enough} "
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tools/img/dit_outlier_lp_hp_complement_amax_by_step"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"cutoffs={cutoffs} conditions={len(conditions)} clip=by_step complement_amax_l1"
    )
    rows: list[StepRow] = []
    ...
    lines.append(
        f"sites with massive L1={n_target}  remaining_ch_full={n_expand}  "
        f"insufficient_skip={n_short}  (summed over conditions x steps, massive path)"
    )
    print("\n=== verdict ===")
    for line in lines:
        print(line)
    csv_path = args.output_dir / "outlier_lp_hp_complement_amax_by_step.csv"
    verdict_path = args.output_dir / "hypothesis.txt"
    _write_csv(rows, csv_path)
    verdict_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    for cutoff in cutoffs:
        _plot_metrics(rows, cutoff, args.output_dir / f"outlier_lp_hp_complement_amax_by_step_k{cutoff}.png")
    _plot_channel_counts(rows, args.output_dir / "outlier_lp_hp_complement_amax_by_step_counts.png")
